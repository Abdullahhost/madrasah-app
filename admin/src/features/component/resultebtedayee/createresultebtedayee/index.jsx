...
const CreateEbtedayeeResult = () => {
  ...
  const handleChange = (e) => {
    setTeacherData((prev) => ({ ...prev, [e.target.name]: e.target.value, pdfFile: pdfFilePic }))
  }
  ...
  return (
    <form onSubmit={handleSubmit} encType='multipart/form-data' className="create_teacher">
      <div className="image_section">
        ...
        <embed className='teacher_profile' src={pdfFilePic ? URL.createObjectURL(pdfFilePic) : 'pdfimage.png'}  />
      </div>
      <div className="create_section_left">

        <div className='profile_uploader'>
          <label className='pdfFiler_uploader_icon'><DriveFolderUploadOutlined /></label>
          <input type="file" accept='.pdf' name='pdfFile' required onChange={handleFileChange} />
        </div>
