...
const FeaturesCreate = () => {
  ...
  const handleChange = (e) => {
    setFeaturesData((prev) => ({ ...prev, [e.target.name]: e.target.value, profile: profilePic }))
  }
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post('https://madrasah-app.onrender.com/alimfeatures/create', formData, { withCredentials: true });
      navigate(-1)
    } catch (err) {
  ...
  return (
    <form onSubmit={handleSubmit} encType='multipart/form-data' className="create_teacher">
      ...
      <div className="create_section_left">
        ...
        <p>Features title</p>
        <input required type="text" name='title' onChange={handleChange} placeholder="Features title here" />
 
        <p>Number Of Teacher</p>
        <input required type="number" name='numberOfTeacher' onChange={handleChange} placeholder=" Number Of Teacher here" />


        <p>Established</p>
        <input required type="text" name='established' onChange={handleChange} placeholder="" />
      ...
      <div className="create_section_right">
        ...
        <p>Description</p>
        <textarea cols={40} rows={13}  name='description' onChange={handleChange} style={{ padding: '1rem', fontSize: '.9rem', fontFamily: 'sans-serif', textAlign: 'justify' }} ></textarea> 
