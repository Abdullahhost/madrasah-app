  ...
  const handleClick = async () => {
    ...
    try {
      const res = await axios.post(
        "https://madrasah-app.onrender.com/auth/login",
        userInfo,
      ...
      {getData.loading ? (
        ...
        <section className="login_section">
          <div className="box">
            ...
            <div className="container">
              <div className="form">
                ...
                <p
                  className="forget"
                  style={{ marginBottom: ".6rem", color: "#555" }}
                >
                  Continue without login or sign up as a Gest mode ?{" "}
                  <NavLink to={"/"}>Click here</NavLink>
                </p>
                <p className="forget">
                  Dont Have not an account ? <Link to={"/signup"}>Sign up</Link>
                </p>
              </div>
            </div>
          </div>
          <div className="alertBox">
            {getData.error && (
              <h1 style={{ color: "#fff" }}>{getData.error.message}</h1>
            )}
          </div>
        </section>
      )}
